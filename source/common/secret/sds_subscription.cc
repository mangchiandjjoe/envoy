...
SdsSubscription::SdsSubscription(Config::SubscriptionStats stats,
    ...
    : Http::RestApiFetcher(
      ...
      stats_(stats), local_info_(local_info) {
  ...
  // TODO(jaebong): Add support for multiple clusters, #1170.
...
void SdsSubscription::parseResponse(const Http::Message& response) {
  ...
  callbacks_->onConfigUpdate(resources, version_info_);
}

void SdsSubscription::onFetchComplete() { ENVOY_LOG(debug, "sds: fetch complete"); }
