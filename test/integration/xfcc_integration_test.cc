  ...
  std::string json_mtls = R"EOF(
{
  ...
  return Network::TransportSocketFactoryPtr{new Ssl::ClientSslSocketFactory(
      std::make_unique<Ssl::ClientContextConfigImpl>(*loader, secret_manager_), *context_manager_,
      *client_stats_store)};
}
  ...
  std::string json = R"EOF(
{
  ...
  return std::make_unique<Ssl::ServerSslSocketFactory>(
      std::make_unique<Ssl::ServerContextConfigImpl>(*loader, secret_manager_), *context_manager_,
      *upstream_stats_store, std::vector<std::string>{});
}
