...
void testUtil(const std::string& client_ctx_json, const std::string& server_ctx_json,
  ...
  Secret::SecretManagerImpl secret_manager(server);
  ...
  Ssl::ServerSslSocketFactory server_ssl_socket_factory(
      std::make_unique<ServerContextConfigImpl>(*server_ctx_loader, secret_manager), manager,
  ...
  Ssl::ClientSslSocketFactory client_ssl_socket_factory(
      std::make_unique<ClientContextConfigImpl>(*client_ctx_loader, secret_manager), manager,
      stats_store);
...
const std::string testUtilV2(const envoy::api::v2::Listener& server_proto,
  ...
  Ssl::ServerSslSocketFactory server_ssl_socket_factory(
      std::make_unique<ServerContextConfigImpl>(filter_chain.tls_context(), secret_manager),
  ...
  ClientSslSocketFactory client_ssl_socket_factory(
      std::make_unique<ClientContextConfigImpl>(client_ctx_proto, secret_manager), manager,
      stats_store);

  ClientContextConfigImpl client_ctx_config(client_ctx_proto, secret_manager);
  ClientContextPtr client_ctx(
...
TEST_P(SslSocketTest, FlushCloseDuringHandshake) {
  ...
  Json::ObjectSharedPtr server_ctx_loader = TestEnvironment::jsonLoadFromString(server_ctx_json);
  ContextManagerImpl manager(runtime, *secret_manager_.get());
  Ssl::ServerSslSocketFactory server_ssl_socket_factory(
      std::make_unique<ServerContextConfigImpl>(*server_ctx_loader, *secret_manager_.get()), manager,
      stats_store, std::vector<std::string>{});
...
TEST_P(SslSocketTest, HalfClose) {
  ...
  Json::ObjectSharedPtr client_ctx_loader = TestEnvironment::jsonLoadFromString(client_ctx_json);
  ClientSslSocketFactory client_ssl_socket_factory(
      std::make_unique<ClientContextConfigImpl>(*client_ctx_loader, *secret_manager_.get()), manager,
      stats_store);
  Network::ClientConnectionPtr client_connection = dispatcher.createClientConnection(
      socket.localAddress(), Network::Address::InstanceConstSharedPtr(),
      client_ssl_socket_factory.createTransportSocket(), nullptr);
...
void testTicketSessionResumption(const std::string& server_ctx_json1,
  ...
  Ssl::ServerSslSocketFactory server_ssl_socket_factory1(
      std::make_unique<ServerContextConfigImpl>(*server_ctx_loader1, secret_manager), manager,
      stats_store, server_names1);
  Ssl::ServerSslSocketFactory server_ssl_socket_factory2(
      std::make_unique<ServerContextConfigImpl>(*server_ctx_loader2, secret_manager), manager,
...
TEST_P(SslSocketTest, ClientAuthCrossListenerSessionResumption) {
  ...
  Ssl::ServerSslSocketFactory server2_ssl_socket_factory(
      std::make_unique<ServerContextConfigImpl>(*server2_ctx_loader, *secret_manager_.get()), manager,
      stats_store, std::vector<std::string>{});
...
public:
  void initialize() {
    ...
    manager_.reset(new ContextManagerImpl(runtime_, secret_manager));
    server_ssl_socket_factory_.reset(
        new ServerSslSocketFactory(
            std::make_unique<ServerContextConfigImpl>(*server_ctx_loader_, secret_manager),
    ...
    client_ssl_socket_factory_.reset(
        new ClientSslSocketFactory(
            std::make_unique<ClientContextConfigImpl>(*client_ctx_loader_, secret_manager),
